...
def test_api_alerts_endpoint(monkeypatch):
    monkeypatch.setattr(config, "API_KEY", None)
    storage.record_alert_event(1, "item", "price", "dropped 10%", ["discord"], "k")
    with TestClient(app) as client:
        data = client.get("/api/alerts").json()
        assert any(e["message"] == "dropped 10%" for e in data)


def test_dashboard_shows_sent_alerts_section():
    with TestClient(app) as client:
        assert "Sent alerts" in client.get("/").text
